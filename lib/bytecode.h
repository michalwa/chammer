...
#define EACH_OPCODE(_)                                                                            \
    ...
    _(OP_DUP, 0x12)       /* duplicate operand */                                                 \
    _(OP_POP, 0x13)       /* remove operand off the stack */                                      \
    _(OP_SWAP, 0x14)      /* swap 2 operands */                                                   \
    _(OP_PUSHINT, 0x20)   /* push int operand */                                                  \
    _(OP_PUSHSTR, 0x21)   /* push string operand */                                               \
    _(OP_PUSHTRUE, 0x22)  /* push the value `true` */                                             \
    _(OP_PUSHFALSE, 0x23) /* push the value `false` */                                            \
    _(OP_MAKECLS, 0x30)   /* look up function, store N captures, and push closure operand */      \
    _(OP_CALLVAL, 0x31)   /* call operand (e.g. closure) with N args */                           \
    _(OP_BIND, 0x32)      /* bind a monadic value to a closure */                                 \
    _(OP_CHKTUPLE, 0x50)  /* check if operand is an N-tuple; if not, pop; then push bool */       \
    _(OP_TUPLEGET, 0x51)  /* push N-th element of tuple on top of the stack (don't pop) */        \
    _(OP_MAKETUPLE, 0x52) /* pop N operands and make a tuple, top-down order */                   \
    _(OP_MAKELIST, 0x53)  /* push `nil` and N times: pop 2 operands and make a `cons` */          \
    _(OP_CHKNIL, 0x54)    /* check if operand is a `nil`; if not, pop; then push bool */          \
    _(OP_CHKCONS, 0x55)   /* check if operand is a `cons`; if not, pop; then push bool */         \
    _(OP_UNCONS, 0x56)    /* pop top `cons` and push `tail` and `head` separately */              \
    _(OP_CONCAT, 0x57)    /* pop 2 list operands and push their concatenation, bottom-up order */ \
    _(OP_EQ, 0x58)        /* pop 2 operands and push a bool indicating equality */                \
    _(OP_LOADEXT, 0x80)   /* pop string operand and load an external symbol with that name */     \
    _(OP_YIELD, 0xF0)     /* yield control to an effect operand */                                \
...
void bytecode_put_chktuple(Buffer *, uint16_t len);
